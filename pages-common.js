 ...
 * Professionelle Hilfsfunktionen (Toast, Cart, Wishlist, Auth)
 */
...
function showToast(msg, type = 'success') {
  ...
  t.id = 'gw-toast';
  t.textContent = msg;
  const bg = type === 'error' ? '#e63946' : '#b8860b';
...
function updateMiniCart() {
  ...
  const cart = getCart();
  if (cart.length === 0) {
    body.innerHTML = `<div class="cart-empty-state"><strong>Ihr Warenkorb ist leer</strong><p>Entdecken Sie unsere Produkte.</p><a href="kategorie.html" class="btn-primary" style="margin-top:16px;display:inline-block">Jetzt einkaufen</a></div>`;
  } else {
      ...
      <div class="cart-sidebar-item">
        ...
        <div class="cart-sidebar-info">
          ...
          <div class="cart-qty-row">
            ...
            <button class="qty-btn" onclick="changeQty(${item.id},1);updateMiniCart()">+</button>
            <button class="cart-remove-btn" onclick="removeFromCart(${item.id});updateMiniCart()">Entfernen</button>
          </div>
        </div>
      </div>`).join('');
  }
...
function toggleWishById(id, btn) {
  ...
  if (list.includes(id)) {
    ...
    if (btn) { btn.textContent = 'Merken'; btn.classList.remove('wished'); }
    showToast('Aus der Wunschliste entfernt.', 'info');
  } else {
    list.push(id);
    if (btn) { btn.textContent = 'Merken ✓'; btn.classList.add('wished'); }
...
function updateAuthLinks() {
  ...
  document.querySelectorAll('.hdr-account-link').forEach(l => { l.style.display = user ? '' : 'none'; if (user) l.title = user.name; });
}
function logoutUser() {
...
function setupSearch(inputEl, dropdownEl, formEl, products) {
  ...
  function doSearch(q) {
    ...
    const results = products.filter(p => p.name.toLowerCase().includes(ql) || p.category.toLowerCase().includes(ql) || (p.shortName && p.shortName.toLowerCase().includes(ql))).slice(0, 8);
    if (results.length === 0) {
      ...
      results.forEach(p => {
        html += `<div class="search-result-item" onclick="location.href='produkt.html?slug=${p.slug}'">
          <div class="search-result-img"><img src="${p.image || 'snowflake.svg'}" alt="" onerror="this.src='snowflake.svg'"></div>
  ...
  formEl.addEventListener('submit', e => { const q = inputEl.value.trim(); if (q) { e.preventDefault(); window.location = 'kategorie.html?q=' + encodeURIComponent(q); } });
}

async function initPage() {
  if (!window._gwProducts) {
    try { const r = await fetch('products.json'); window._gwProducts = await r.json(); } catch (e) { window._gwProducts = []; }
  }
  updateCartBadge();
  updateWishBadge();
  updateMiniCart();
  ...
  if (msToggle && msBar) {
    msToggle.onclick = () => { const open = msBar.style.display === 'block'; msBar.style.display = open ? 'none' : 'block'; if (!open && msi) msi.focus(); };
  }

  document.getElementById('newsletterForm')?.addEventListener('submit', e => { e.preventDefault(); showToast('Erfolgreich angemeldet!'); e.target.reset(); });
}
